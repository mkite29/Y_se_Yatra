...
glColor3f(1.0,1.0,1.0);
     glRasterPos2f(-815,185);
     declare("   OOPS PROJECT");
glEnd();
...
int c_menu;
	...
	myinit();
	admin_menu=glutCreateMenu(main_menu);

	glutAddMenuEntry("Add data",1);
	glutAddMenuEntry("Display",2);
	glutAddMenuEntry("Search",3);
	glutAddMenuEntry("Modify",4);
	glutAddMenuEntry("Delete",5);

	user_menu=glutCreateMenu(menu);
	glutAddMenuEntry("Book Ticket",9);
	glutAddMenuEntry("Display",6);

	glutCreateMenu(main_menu);
    glutAddSubMenu("Adminstrator",admin_menu);
    glutAddSubMenu("Customer",user_menu);
